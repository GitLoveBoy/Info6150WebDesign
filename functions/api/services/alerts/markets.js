    ...
    if (total_market_cap > 0) {
      ...
      if (hour_exceed || day_exceed) {
        ...
        data.forEach((d, i) => {
          ...
          telegram_message += `<a href="${website}/token/${id}">${symbol ? symbol.toUpperCase() : name}</a> <b>${currency_symbol}${number_format(current_price, `0,0${current_price >= 100 ? '' : current_price >= 1 ? '.00' : '.00000000'}`)}</b> <pre>${number_format(p