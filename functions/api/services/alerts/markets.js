  ...
  if (Number(now.minutes()) === 0) {
    if (Number(now.hours()) % 4 === 0) {
      ...
      if (data) {
        ...
        telegram.push(`Today's 👑🟠 <a href="${website}/token/${id}">${symbol ? symbol.toUpperCase() : name}</a> price <b>${currency_symbol}${number_format(current_price, `0,0${current_price >= 100 ? '' : current_price >= 1 ? '.00' : '.00000000'}`)}</b> <pre>${number_fo